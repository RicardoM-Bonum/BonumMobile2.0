...
import NoMoreSessions from '../NoMoreSessions';
import { Button } from '@rneui/base';
import { PrimaryButton } from '../../../../components/Buttons';

function AvailableSchedule({ scrollView, navigation, handleSchedule }) {
  const { loading, callEndpoint } = useFetchAndLoad();
  const [loadingReset, setLoadingReset] = useState(false);
  const [noMoreSessions, setNoMoreSessions] = useState(false);
  const [schedule, setSchedule] = useState([]);
  const { coach, timezone, additionalSessions, cohort, sessions } = useSelector(
    (state) => state.user
  );
  ...
  const coachAvailability = async () => {
    try {
      if (sessions.length >= cohort?.program + additionalSessions) {
        setNoMoreSessions(true);
        return;
      }
