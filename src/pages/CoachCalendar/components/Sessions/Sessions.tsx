...
import { size, map, orderBy } from 'lodash';
...
const Sessions = ({ date, sessionsToDisplay }) => {
  ...
  const displayedSessions = orderBy(sessionsToDisplay, ['date', 'asc']);

  useEffect(() => {}, [sessionsToDisplay]);

  return size(displayedSessions) > 0 ? (
    <View styles={styles.wrapper}>
      <Text style={styles.title}>{dateToLongDate(date)}</Text>
      {map(displayedSessions, (session) => (
        <View style={getItemStyle(session)} key={session._id}>
          <Text style={styles.text}>
            Sesión con tu coachee {session?.coachee?.name}{' '}
            {session.coachee.lastname}
          </Text>
